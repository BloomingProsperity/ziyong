...
class EventParser:
    ...
    def _create_streaming_event(self, sse_msg: SSEMessage) -> Optional[StreamingEvent]:
        ...
        try:
            ...
            data, _ = decoder.raw_decode(sse_msg.data)
    ...
    async def flush(self) -> AsyncIterator[StreamingEvent]:
        ...
        if self.buffer.strip():
            ...
            # Insert \n\n before each "event:" line to properly separate
            # multiple SSE events that may be concatenated in the buffer
            lines = self.buffer.split("\n")
            fixed_parts = []
            for line in lines:
                if line.startswith("event:") and fixed_parts:
                    fixed_parts.append("\n")
                fixed_parts.append(line + "\n")
            self.buffer = "".join(fixed_parts) + "\n"
